...
export default function Signup() {
  ...
  const [getVerifier, { data: verifierData }] = useLazyQuery(GET_UNIQUE_VERIFIER);
  ...
  const [signup, {loading}] = useMutation(SIGNUP, {
    ...
    onCompleted: async (data) => {
      ...
      try {
        const { data: verifierData } = await getVerifier({ variables: { email } });
        const verifier = verifierData?.getUserVerificationToken.uniqueverifier;
        await sendEmail(email, firstName, verifier);
      } catch (error) {
